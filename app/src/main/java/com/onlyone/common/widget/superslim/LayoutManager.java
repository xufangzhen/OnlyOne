package com.onlyone.common.widget.superslim;
...
public class LayoutManager extends RecyclerView.LayoutManager {
    ...
    public RecyclerView.LayoutParams generateLayoutParams(Context c, AttributeSet attrs) {
        ...
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.LOLLIPOP) {
            ...
            isString =
                    a.getType(R.styleable.superslim_LayoutManager_slm_section_sectionManager) == TypedValue.TYPE_STRING;
        }
        String sectionManager = null;
        int sectionManagerKind;
        if (isString) {
            sectionManager = a.getString(R.styleable.superslim_LayoutManager_slm_section_sectionManager);
            ...
            sectionManagerKind =
                    a.getInt(R.styleable.superslim_LayoutManager_slm_section_sectionManager, SECTION_MANAGER_LINEAR);
        }
        a.recycle();
    ...
    public int scrollVerticallyBy(int dy, RecyclerView.Recycler recycler, RecyclerView.State state) {
        ...
        if (isDirectionEnd) {
            ...
            final int endEdge =
                    slm.getLowestEdge(params.getTestedFirstPosition(), getChildCount() - 1, getDecoratedBottom(end));
    ...
    public void layoutDecorated(View child, int left, int top, int right, int bottom) {
        final ViewGroup.MarginLayoutParams lp = (ViewGroup.MarginLayoutParams) child.getLayoutParams();
        super.layoutDecorated(child, left + lp.leftMargin, top + lp.topMargin, right - lp.rightMargin,
    ...
    private int fillNextSectionToStart(int leadingEdge, int markerLine, LayoutState state) {
        ...
        if (sd.hasHeader) {
            ...
            markerLine =
                    layoutHeaderTowardsStart(header, leadingEdge, markerLine, headerOffset, sectionBottom, sd, state);
    ...
    private int getDirectionToPosition(int targetPosition) {
        SectionData sd = new SectionData(this, getChildAt(0));
        final View startSectionFirstView = getSlm(sd).getFirstVisibleView(sd.firstPosition, true);
    ...
    private View getHeaderOrFirstViewForSection(int sfp, Direction direction, LayoutState state) {
        View view = findAttachedHeaderOrFirstViewForSection(sfp, direction == Direction.START ? 0 : getChildCount() - 1,
                direction);
        if (view == null) {
    ...
    private boolean isOverscrolled(LayoutState state) {
        ...
        final boolean lastVisibleBeforeEnd = getDecoratedBottom(lastVisibleView) < getHeight() - getPaddingBottom();
     ...
     */
    private int layoutHeaderTowardsStart(View header, int leadingEdge, int markerLine, int offset, int sectionBottom,
    ...
    private int updateHeaderForStart(View header, int leadingEdge, int markerLine, SectionData sd, LayoutState state) {
        ...
        for (int i = sli == -1 ? 0 : sli; i < getChildCount(); i++) {
            ...
            if (params.getTestedFirstPosition() != sd.firstPosition) {
                View first =
                        findAttachedHeaderOrFirstViewForSection(params.getTestedFirstPosition(), i, Direction.START);
                if (first == null) {
    ...
    public static class LayoutParams extends RecyclerView.LayoutParams {
        ...
        public LayoutParams(Context c, AttributeSet attrs) {
            ...
            TypedArray a = c.obtainStyledAttributes(attrs, R.styleable.superslim_LayoutManager);
            isHeader = a.getBoolean(R.styleable.superslim_LayoutManager_slm_isHeader, false);
            //noinspection ResourceType
            headerDisplay = a.getInt(R.styleable.superslim_LayoutManager_slm_headerDisplay, DEFAULT_HEADER_DISPLAY);
            mFirstPosition = a.getInt(R.styleable.superslim_LayoutManager_slm_section_firstPosition, NO_FIRST_POSITION);

            // Header margin types can be dimension or integer (enum).
            if (Build.VERSION.SDK_INT < Build.VERSION_CODES.LOLLIPOP) {
                TypedValue value = new TypedValue();
                a.getValue(R.styleable.superslim_LayoutManager_slm_section_headerMarginStart, value);
                ...
                boolean isDimension;
                isDimension = a.getType(R.styleable.superslim_LayoutManager_slm_section_headerMarginStart)
                        == TypedValue.TYPE_DIMENSION;
                loadHeaderStartMargin(a, isDimension);

                isDimension = a.getType(R.styleable.superslim_LayoutManager_slm_section_headerMarginEnd)
                        == TypedValue.TYPE_DIMENSION;
                loadHeaderEndMargin(a, isDimension);

                boolean isString = a.getType(R.styleable.superslim_LayoutManager_slm_section_sectionManager)
        ...
        private void loadHeaderEndMargin(TypedArray a, boolean isDimension) {
            if (isDimension) {
                ...
                headerMarginEnd =
                        a.getDimensionPixelSize(R.styleable.superslim_LayoutManager_slm_section_headerMarginEnd, 0);
            } else {
                headerEndMarginIsAuto = true;
            }
        }
        ...
        private void loadHeaderStartMargin(TypedArray a, boolean isDimension) {
            if (isDimension) {
                ...
                headerMarginStart =
                        a.getDimensionPixelSize(R.styleable.superslim_LayoutManager_slm_section_headerMarginStart, 0);
            } else {
                headerStartMarginIsAuto = true;
            }
        }
        ...
        private void loadSlm(TypedArray a, boolean isString) {
            if (isString) {
                ...
                sectionManagerKind = a.getInt(R.styleable.superslim_LayoutManager_slm_section_sectionManager,
                        SECTION_MANAGER_LINEAR);
            }
        }

        @IntDef(flag = true, value = {
                HEADER_INLINE, HEADER_ALIGN_START, HEADER_ALIGN_END, HEADER_STICKY, HEADER_OVERLAY
        })
        ...
        private class MissingFirstPositionException extends RuntimeException {
            ...
            private static final long serialVersionUID = 4724137408359559108L;
        ...
        private class InvalidFirstPositionException extends RuntimeException {
            ...
            private static final long serialVersionUID = -9086023978569368721L;
    ...
    private class NotYetImplementedSlmException extends RuntimeException {
        ...
        private static final long serialVersionUID = 4579002031531257359L;
    ...
    private class UnknownSectionLayoutException extends RuntimeException {
        ...
        private static final long serialVersionUID = 7654767606148129479L;
