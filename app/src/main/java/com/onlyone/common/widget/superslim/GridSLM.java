...
public class GridSLM extends SectionLayoutManager {
    ...
    public int computeHeaderOffset(int firstVisiblePosition, SectionData sd, LayoutState state) {
         ...
         */
        int areaAbove = 0;
        for (int position = sd.firstPosition + 1; areaAbove < sd.headerHeight && position < firstVisiblePosition;
     ...
     */
    public int fillRow(int markerLine, int anchorPosition, LayoutManager.Direction direction, boolean measureRowItems,
    ...
    private void calculateColumnWidthValues(SectionData sd) {
        ...
        if (mColumnWidth == 0) {
            Log.e("GridSection", "Too many columns (" + mNumColumns + ") for available width" + availableWidth + ".");
        }
    }
     ...
     * @param state Layout state.
     */
    private void layoutChild(LayoutState.View child, int top, int col, int rowHeight, SectionData sd,
            LayoutState state) {
        final int height;
    ...
    public static class LayoutParams extends LayoutManager.LayoutParams {
        ...
        public LayoutParams(Context c, AttributeSet attrs) {
            ...
            mColumnWidth = a.getDimensionPixelSize(R.styleable.superslim_GridSLM_slm_grid_columnWidth, -1);
            a.recycle();
        }
